/** @jsx jsx */
import { React, ReactRedux, jsx, QueriableDataSource, classNames, DataRecord, Immutable, ImmutableObject, MessageManager, DataRecordsSelectionChangeMessage, AllWidgetProps } from 'jimu-core'
   ...
   // Custom result search - SonPA
    const handleProccessResultSearch = async (record) => {
      ...
      if (record.length > 0) {
        ...
        const div = <RenderTemplateAttributes
        allProps={props.propsAll} defaultMessages = {defaultMessages}
        handleClickHide={handleClickHide} results={feature} config={config} />;
